...
def build_model(X_train,Y_train):
    ...
    model = CatBoostRegressor(iterations=1000, learning_rate=0.05, depth=10, loss_function='RMSE', cat_features=[])
...
for c in ['Open','High','Low','Close']:
    ...
    plt.tight_layout()
    # plt.show()
    plt.savefig(f"catboost_{c}_{name}.png")
    plt.close()
    plt.cla()
